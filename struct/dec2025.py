...
def _min_change(amount, coins, memo):
  ...
  ans = _min_change(amount, coins, {})
  return -1 if ans == float('inf') else ans
  #receive a list of integers and an integers
  #return amount of coins needed to make the amount

  #ex: 6[2, 4] => 2  of 4 + 2

  #iterate through the coins
  #add amount of coins to the min coins
  #base cases if amount is 0 or reached a negative value
...
def _min_change(amount, coins, memo):
  ...
  min_coins = float('inf')
  for coin in coins:
    new_change = 1 + _min_change(amount- coin, coins, memo)
    
    
    
    if new_change < min_coins:
      min_coins = new_change
...
def max_path_sum(grid):
  return _max_path_sum(grid, 0, 0, {})
  #receive a grid
  #return the max sum found in the grid
  #ex: 


  #base case:
  #if we are bottom corner, return the cell value of it

  #add the cell value to each 
  #if sum > max_sum, update max_summ
  #return max _ sum
...
def tree_includes(root, target):
  #receive a root node and a target
  #return a boolean
...
def tree_value_count(root, target):
  #receive the root node
  #return the count of how many times target is seen in tree 
...
def max_path_sum(root):
  #receive the root node of a value
  #return max sum seen
  #iterate through each node
  #add sum 
  #return max val seen
