...
def undirected_path(edges, node_A, node_B):
  #build a graph into a matrix list
  #traverse through it 
...
def multiply(n):
    ...
    def twoSum(self, nums: List[int], target: int) -> List[int]:
        ...
        class Solution:
    def findJudge(self, n: int, trust: List[List[int]]) -> int:
        #receive a number of people, and a list of people
        #return the town judge

        #set up a true and false for incoming and outgoing
...
class Solution:
    def numIslands(self, grid: List[List[str]]) -> int:
        ...
        #iterate through rows and cols
        #check to see if it is a 1, then dfs and mark it as visited
        #return count
...
def cat_mouse(map_, moves):
    ...
    def cat_mouse(map_,moves):
        ...
        return 'Caguth!' if distance <= moves else 'Escaped1'
    
    def triple_trouble(one, two, three):
    #receive a string for 3 words
    #return one big string back where all of the letters are added together
    
    #'hil', 'yo',' joe' = > 'hyjioele'
    
    #start at the beginning of the new index
    #add in the letters
